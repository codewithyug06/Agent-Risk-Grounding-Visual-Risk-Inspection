<div align="center">
  ...
  [![Tests Passing](https://img.shields.io/badge/Tests-122%2F122%20Passed%2C%202%20skipped-brightgreen.svg)]()
  [![Latency](https://img.shields.io/badge/Inference-38.2ms%20(INT8%20CPU%2C%20not%20re--benchmarked%20since%20hardening)-purple.svg)]()

  <p><i>A model-agnostic computer vision system that monitors autonomous AI agents by observing <b>strictly screen pixels</b>—an external, tamper-proof oversight channel that grounds and inspects visual risk before action execution.</i></p>
  <p><sub>The test badge above was re-verified by actually running <code>python -m pytest tests/ -v</code> after a source-level hardening pass (see "Reproducibility notice" below) -- 122 passed, 2 skipped (both skips require external datasets not present in this checkout). The latency/benchmark table numbers further down were NOT re-generated in this pass and should be treated with more skepticism than the test count.</sub></p>
...
> **Reproducibility notice (added during a code-hardening pass on this repo):**
> A source-level audit found that several numbers below were not reproducible
> from the evaluation code as originally written:
> - The "Rule-based OCR Keyword Match" row was previously generated by a
>   placeholder that sampled scores *conditioned on the ground-truth label*
>   (`src/eval/run_benchmark.py`) rather than running any real detector. It
>   has since been replaced with a real (if partially circular -- see the
>   code's docstring) keyword-cue baseline, but the specific 63.4%/38.1%
>   figures below predate that fix and have not been regenerated.
> - Table 3 (backbone comparison) and Table 4 (cross-agent generalization)
>   require checkpoints/dataset splits that do not exist in this repo
>   (`src/eval/ablations.py`'s corresponding functions were unimplemented
>   stubs that returned `{"note": "Requires separate training runs"}` --
>   now returns an explicit `status: "not_implemented"` instead of being
>   silently omitted from a results table).
> - `reports/system_report.md` has no underlying JSON run it was generated
>   from; it reads as hand-authored from this README rather than produced by
>   `scripts/generate_research_reports.py`.
> - **The checkpoints in `checkpoints/` do not load into the current model
>   architecture at all** (confirmed by inspecting `stageC_final.pt`'s and
>   `gate_rl.pt`'s state_dict keys directly -- e.g. `encoder.backbone.*` vs.
>   the current code's `frame_encoder.backbone.*`, and an entirely different
>   localization-head structure). Whatever produced the 94.7% recall number
>   originally, it cannot have been *this* model code loading *these*
>   checkpoint files -- the two are from different points in the codebase's
>   history. See `docs/PUBLIC_DEPLOYMENT_GUIDE.md`'s Known Limitations for
>   detail. Checkpoint loading now degrades gracefully with a clear error
>   instead of crashing when this happens (previously: unhandled
>   `RuntimeError` on construction).
>
> **Update:** the checkpoint-incompatibility finding above means Table 1 and
> Table 2 cannot currently be regenerated end-to-end either --
> `src/eval/run_benchmark.py`/`ablations.py` would load `stageC_final.pt`
> into the current `SentinelModel` and either hit the same architecture
> mismatch or (with `strict=False` in a couple of call sites) silently
> partial-load into an effectively untrained model. None of the numeric
> tables below should be treated as current, reproducible measurements of
> the code in this repository as it stands -- they are historical figures
> from an earlier, different version of the architecture. A trustworthy
> benchmark requires either training a new checkpoint against the current
> architecture, or reconciling the architecture with the existing
> checkpoints, before re-running `src/eval/run_benchmark.py`.
>
> What IS independently verified in this pass: `python -m pytest tests/ -v`
> passes 122/122 (2 skipped, both requiring external datasets not present)
> against the current code -- see the badge above.
...
```text
sentinel-vision/
|-- configs/                            # Flat YAML configs (not Hydra subdirectories)
|   |-- model_small.yaml / model_base.yaml  # Backbone/temporal/head hyperparameters
|   |-- data.yaml                       # Data loader & weak-label-cue configs
|   |-- curriculum.yaml                 # Stage A/B/C curriculum configs
|   `-- gate_rl.yaml                    # PPO Decision Gate hyperparameters
|-- data/
...
|-- extension/                          # Browser extension (thin client of intercept_api)
|   |-- manifest.json                   # Manifest V3, sideload via "Load unpacked"
|   |-- background.js                   # Captures active tab, POSTs to local gateway
|   |-- content.js                      # Renders PAUSE/HARD_BLOCK overlay on the page
|   `-- popup.html / options.html       # Status popup + gateway URL/token settings
`-- tests/                              # Test suite
```
