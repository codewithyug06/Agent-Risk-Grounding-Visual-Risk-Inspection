...
"""
        for r in records:
            html += f"""
    <div class="card">
        <img class="thumb" src="file:///{r.get('screenshot_path')}" alt="Evidence Screenshot">
        <div class="info">
            <span class="badge">{r.get('decision')}</span>
            <h3>{r.get('incident_id')} — {r.get('category').upper()} (Risk: {r.get('risk_score', 0):.2%})</h3>
            <p><b>Agent:</b> {r.get('agent_name')} | <b>Time:</b> {r.get('timestamp')}</p>
            <p><b>Attempted Action:</b> <code>{r.get('action_attempted')}</code></p>
            <p><b>Visual Oversight Analysis:</b> {r.get('reasoning')}</p>
        </div>
    </div>
"""
        html += "</body></html>"
