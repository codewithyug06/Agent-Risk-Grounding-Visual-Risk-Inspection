...
class BenchmarkRunner:
    ...
    def _evaluate_rule_based(self) -> Dict[str, Any]:
        """Evaluate rule-based OCR baseline (simulated)."""
        # This is a placeholder - in practice you'd run OCR on screenshots
        # For now, simulate with keyword matching on action strings
        # Since we don't have actions in this dataloader, return dummy
        all_labels = []
        ...
        # Simulate rule-based: 50% recall, 50% precision roughly
        # This is just for benchmark structure
        n = len(all_labels)
        rule_scores = np.zeros(n)

        # Flag as harmful if label is harmful (oracle upper bound)
        # Real rule-based would be much worse
        rule_scores[all_labels == 1] = 0.7 + np.random.rand(np.sum(all_labels)) * 0.2
        rule_scores[all_labels == 0] = np.random.rand(np.sum(all_labels == 0)) * 0.4

        return {"safety_metrics": compute_safety_metrics(rule_scores, all_labels)}
...
def run_benchmark(config: DictConfig, quick: bool = False) -> Dict[str, Any]:
    ...
    # Load model
    checkpoint_path = config.get("checkpoint", "checkpoints/stage_c/best.pt")
